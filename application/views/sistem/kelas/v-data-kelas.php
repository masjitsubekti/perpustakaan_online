...
<table class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
    ...
    <tr class="tr-head">
        ...
        <th width="50%" class="sortable" id="column_nama_kelas" data-sort="" onclick="sort_table('#column_nama_kelas','nama_kelas')">Nama Kelas</th>
    ...
    <tr>
        ...
        <td style="text-align:center; padding-top:5px;">
            <a href="javascript:;" data-id="<?=$row->id_kelas?>" data-name="<?=$row->nama_kelas?>" class="btn btn-sm btn-warning btn-ubah" data-toggle="tooltip" title="Edit <?=$row->nama_kelas?>"><i style="color:#fff;" class="fa fa-edit"></i></a>
			<a href="javascript:;" data-id="<?=$row->id_kelas?>" data-name="<?=$row->nama_kelas?>" class="btn btn-sm btn-danger btn-hapus" data-toggle="tooltip" title="Hapus <?=$row->nama_kelas?>"><i class="fa fa-trash"></i></a>	    
        </td>
    </tr>
    <?php } ?>
    </tbody>
...
<script>
	...
	$('.btn-hapus').click(function (e) {
		...
		Swal.fire({
			...
			text: "Apakah Anda yakin menonaktifkan Kelas  : " + title + " !",
			type: 'warning',
			...
			preConfirm: function () {
				return new Promise(function (resolve) {
					$.ajax({
						method: 'POST',
						dataType: 'json',
						url: site_url + 'Kelas/nonaktifkan',
						data: {
							id: id
						},
