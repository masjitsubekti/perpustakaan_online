...
<table class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
    ...
    <tr class="tr-head">
        ...
        <th width="50%" class="sortable" id="column_nama_bahasa" data-sort="" onclick="sort_table('#column_nama_bahasa','nama_bahasa')">Bahasa</th>
    ...
    <tr>
        ...
        <td style="text-align:center; padding-top:5px;">
            <a href="javascript:;" data-id="<?=$row->id_bahasa?>" data-name="<?=$row->nama_bahasa?>" class="btn btn-sm btn-warning btn-ubah" data-toggle="tooltip" title="Edit <?=$row->nama_bahasa?>"><i style="color:#fff;" class="fa fa-edit"></i></a>
			<a href="javascript:;" data-id="<?=$row->id_bahasa?>" data-name="<?=$row->nama_bahasa?>" class="btn btn-sm btn-danger btn-hapus" data-toggle="tooltip" title="Hapus <?=$row->nama_bahasa?>"><i class="fa fa-trash"></i></a>	    
        </td>
    </tr>
    <?php } ?>
    </tbody>
...
<script>
    ...
    $(".btn-ubah").click(function() {
      ...
      var id = $(this).attr('data-id');
		  $.ajax({
        url: "<?php echo site_url('Bahasa/load_modal/')?>",
        type: 'post',
        dataType: 'html',
              data:{id:id},
        beforeSend: function () {},
    ...
    var title = $(this).attr('data-name');
		...
		Swal.fire({
			title: 'Nonaktifkan Bahasa',
			text: "Apakah Anda yakin menonaktifkan Bahasa  : " + title + " !",
			type: 'warning',
			...
			preConfirm: function () {
				return new Promise(function (resolve) {
					$.ajax({
						method: 'POST',
						dataType: 'json',
						url: site_url + 'Bahasa/nonaktifkan',
						data: {
							id: id
						},
